...
class MainGridPane() extends GridPane
    ...
    // PRIVATE WIDGETS
    private val topHbox = HBox(10)
    private val bottomHbox = HBox(10)

    // PUBLIC WIDGETS
    val tableView = TableView[Note]()
    tableView.getSelectionModel.setSelectionMode(SelectionMode.MULTIPLE)
    val searchField = new TextField  //DOTTY for some reason, `new` is req’d here
    searchField.setPromptText("<search>")
    val textSearchButton = Button("all")
    val tagSearchButton = Button("tags")
    val addNoteButton = Button("add note")
    val deleteNoteButton = Button("delete note")
